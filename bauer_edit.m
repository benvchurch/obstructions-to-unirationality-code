...
ExistingSurfaces:=function(pg,gamma,M: 
    BO:={256,512,768,1152,1280,1536,1728,1792,1920}, moG:=20)
  N:=[* *]; 
...
Pi1:=function(pairsofseqs,gr)
  ...
  grxgr,inG,projG:=DirectProduct(gr,gr);
...
for pair in types do
  ...
  for signature in signatures do
    print signature;
    for ord in [10..20] do
      for G in SmallGroups(ord) do
          // print G;
          list_of_gens := SphGenUpToConj(G, signature);
          count := 0;
          for gens1 in list_of_gens do
            for gens2 in list_of_gens do
              count +:= 1;
              if count gt 10 then break gens1; end if;
              orderPi1 := function(gens1, gens2, G)
                  try
                      t := Cputime();
                      val := Order(Pi1([gens1, gens2], G));
                      if Cputime() - t gt 1 then
                          return -1;
                      end if;
                      return val;
                  catch e
              ...
              if orderPi1(gens1, gens2, G) eq 1 and DiagonalFormNumber([gens1, gens2], G) ge 0 then
                print "FOUND";
                print ComputeBasket(gens1, gens2, G);
                print G;
                print IdentifyGroup(G);
                print gens1;
                print gens2;
                print DiagonalFormNumber([gens1, gens2], G);
              end if;
            end for;
          end for;
      end for;
