...
\title{The Brauer obstruction for the Fermat quartic}
...
%% ------------------------------------------------------------------------
\subsection{Obstruction over $K = \QQ(\sqrt{-3})$ via $\Aut(D_K)$}
\label{sec:aut_D}
%% ------------------------------------------------------------------------

The preceding arguments show that $D$ does not descend to $\QQ$ as a
cover of $C$. One may ask whether the Brauer class $\delta(\eta)$ is
killed by the larger automorphism group of $D$ itself---that is,
whether the image of $\delta(\eta)$ under the map
$H^2(\Gal(K/\QQ),\, K^*) \to H^2(\Gal(K/\QQ),\, \Aut(D_K))$
induced by the inclusion $\langle \iota \rangle \hookrightarrow \Aut(D_K)$
is trivial. If so, $D_K$ would descend from $K$ to $\QQ$
(through this particular extension).

A Magma computation over $\FF_{13}$ (where $\sqrt{-3}$ and $\sqrt{-1}$
both exist) determines the full automorphism group:
\[
  \Aut(D_{\Qbar}) \;\cong\; C_2^2.\operatorname{SL}(2,3)
  \quad (\text{SmallGroup}(96,3)),
\]
with geometric order $96$, confirmed at multiple primes $p = 13, 37$.
Over $K$ alone (where $\sqrt{-1}$ is not available), only $48$
automorphisms are visible: $\Aut(D_K) \cong C_2 \times S_4$
(confirmed at primes $p = 19, 43$ where $\sqrt{-1} \notin \FF_p$).
The full automorphism group is defined over
$\QQ(\sqrt{-3}, i) = \QQ(\zeta_{12})$.

The deck involution $\iota$ is \textbf{central} in $\Aut(D_K)$:
it generates $Z(\Aut(D_K)) \cong \ZZ/2\ZZ$.
The Galois group $\Gal(K/\QQ) = \langle \sigma \rangle$ acts
\emph{trivially} on $\Aut(D_K)$, since $\sigma$ fixes both $Z(\Aut(D_K))$
(the deck involution is canonical) and $\Aut(D_K)/Z(\Aut(D_K)) \cong S_4$
(which is the stabiliser of $\eta$ in $\Aut(C)$, defined over $\QQ$).
Furthermore, $\operatorname{Hom}(S_4, \ZZ/2\ZZ) \cong \ZZ/2\ZZ$ but
the unique lift of the sign character does not split the extension,
as one checks that no element $g \in \Aut(D_K)$ satisfies $g^2 = \iota$:
all $24$ elements of order $4$ square to non-central involutions.

Since $\sigma$ acts trivially on $\Aut(D_K)$, the cocycle
$[\sigma \mapsto \iota] \in H^1(\Gal(K/\QQ),\, \Aut(D_K))$ is a
homomorphism, and it is nontrivial because $\iota$ is central (hence
not conjugate to the identity). Therefore the class $\delta(\eta)$ is
\emph{not killed} in $H^2(\Gal(K/\QQ),\, \Aut(D_K))$, and $D_K$ does
not descend from $K$ to $\QQ$ through this extension.
...
a rational construction on $C$ itself. The field $\QQ(\sqrt{-3})$ is,
in some sense, the simplest extension over which the obstruction becomes
visible.
\end{remark}
