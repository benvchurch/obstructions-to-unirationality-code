...
\end{equation}
The conic $Q$ is the unique (up to scalar) \emph{contact conic} that is
tangent to $C$ at the eight contact points $\{P_i, P_j, P_k, P_l\}$ of the
four bitangents.
\end{theorem}
...
The geometric content of \eqref{eq:star} is: cutting out $Q^2 = 0$ on $C$
gives the divisor $2(P_i + P_j + P_k + P_l)$, which equals the divisor cut
by $L_i L_j L_k L_l$ on $C$ (each line contributes $2 P_m$). Both sides
therefore give effective representatives of $4 K_C$, and \eqref{eq:star} is
the unique linear relation between $L_i L_j L_k L_l$, $Q^2$, and $F$ in
degree $4$.
...
3 & witnessing conic $Q$ for each pair-of-pairs
  ...
  & contact conic \S\ref{sec:quartic-decomp} \\
