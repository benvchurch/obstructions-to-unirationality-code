 ...
 * eta_stabilizer.m
 *
 * Unified computation: Aut(C) orbits on J[2], identification of eta from
 * the quadric decomposition, and direct Stab_{Aut(C)}(eta) computation.
 *
 * Use p = 97 (= 1 mod 24) so F_p has 4th roots of unity AND sqrt(-3).
 *
 * RESULTS:
 * - |Stab_{Aut(C)}(eta)| = 48 = |Aut(D)|/2 (consistent with Aut(D) = 96)
 * - |Orbit of eta| = 2 (= 96/48)
 ******************************************************************************/
...
// =========== SECTION 2: DEGREE-1 PLACES AND BASIS ===========
...
// =========== SECTION 3: AUTOMORPHISMS AND F_2 ACTION ===========
...
// =========== SECTION 4: ORBIT STRUCTURE ON J[2]\{0} ===========
...
// =========== SECTION 5: IDENTIFY ETA FROM QUADRIC DECOMPOSITION ===========
...
// =========== SECTION 6: DIRECT STAB COMPUTATION VIA DIVISOR CRITERION ===========
// Criterion: sigma fixes eta iff sigma*(q1)/q1 = c*h^2
// (all valuations of ratio are even AND half-divisor is principal)

printf "=== DIRECT STABILIZER VIA DIVISOR CRITERION ===\n\n";
...
for k in [1..96] do
    ...
    supp_D := Support(D);
    if #supp_D eq 0 then
        // ratio is a constant -- trivially fixes eta
        Append(~stab_indices_direct, k);
        continue;
    end if;
    ...
    // All valuations even. Check if half-divisor is principal.
    half_D := D div 2;
    cls := half_D @@ m;
    if cls eq Cl!0 then
        Append(~stab_indices_direct, k);
...
printf "|Stab_{Aut(C)}(eta)| = %o (direct criterion)\n", #stab_indices_direct;
printf "|Orbit of eta| = %o (= 96 / %o)\n", 96 div #stab_indices_direct, #stab_indices_direct;
printf "Expected |Aut(D)| = 2 * |Stab| = %o\n\n", 2 * #stab_indices_direct;

printf "Stabilizer elements (%o total):\n", #stab_indices_direct;
for k in stab_indices_direct do
    printf "  %o\n", aut_list[k][2];
end for;
...
// =========== SECTION 7: COMPUTE Aut(D) DIRECTLY FOR COMPARISON ===========
