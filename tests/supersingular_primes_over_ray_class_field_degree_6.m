/*******************************************************************************
 * supersingular_primes_over_ray_class_field_degree_6.m
 *
 * Purpose:
...
E := EllipticCurve([K![1,0,0,0,0,1],K![-1,0,0,0,0,0],K![0,1,0,0,0,0],K![0,-2,0,-35,1,3],K![0,1,0,0,3,0]]);
  ...
  for p in primes do
    if (D mod p) ne 0 then
      ...
      for f in fact do
        pp := f[1];
        if (TraceOfFrobenius(E, pp) mod p) eq 0 then
          print "Characteristic: ", p, "Norm: ", Norm(pp), "mod 7: ", Norm(pp) mod 7, "mod 12: ", Norm(pp) mod 12, "mod 13: ", Norm(pp) mod 13;
          Append(~ss_primes, pp);
        end if;
      end for;
    end if;
